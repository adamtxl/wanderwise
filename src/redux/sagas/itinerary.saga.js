...
function* addItinerary(action) {
    try {
        yield axios.post(`/api/trip/${action.payload.tripId}/itineraries`, action.payload.itinerary);
...
function* updateItinerary(action) {
    try {
        yield axios.put(`/api/itineraries/${action.payload.itineraryId}`, action.payload.itinerary);
        yield put({ type: 'FETCH_ITINERARIES', payload: action.payload.tripId });
    } catch (error) {
